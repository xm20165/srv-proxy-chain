...
function isInvalidHeader  (name, value)  {
...
function redactUrl  (url, passwordReplacement) {
...
function redactParsedUrl  (parsedUrl, passwordReplacement = '<redacted>')  {
    const p = parsedUrl;
    let auth = null;
    if (p.username) {
        if (p.password) {
...
function parseProxyAuthorizationHeader  (header)  {
...
function maybeAddProxyAuthorizationHeader  (parsedUrl, headers)  {
...
module.exports = {parseHostHeader,isHopByHopHeader,isInvalidHeader, parseUrl , redactUrl ,redactParsedUrl , parseProxyAuthorizationHeader , addHeader , PORT_SELECTION_CONFIG , findFreePort , maybeAddProxyAuthorizationHeader };
